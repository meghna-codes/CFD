...
A = spdiags([-1 a11 0;ones(n-2,1)*[-1 2 -1];0 a11 -1],-1:1,n,n)'/h^2;end
...
uE = avg(y)*0; vE = y*0;
%-----------------------------------------------------------------------
Ubc = dt/R*([2*uS(2:end-1)' zeros(nx-1,ny-2) 2*uN(2:end-1)']/hx^2+...
[uW;zeros(nx-3,ny);uE]/hy^2);
Vbc = dt/R*([vS' zeros(nx,ny-3) vN']/hx^2+...
...
Lu = speye((nx-1)*ny)+dt/R*(kron(speye(ny),K1(nx-1,hx,2))+...
kron(K1(ny,hy,3),speye(nx-1)));
peru = symamd(Lu); Ru = chol(Lu(peru,peru)); Rut = Ru';

Lv = speye(nx*(ny-1))+dt/R*(kron(speye(ny-1),K1(nx,hx,3))+...
...
quiver(x,y,(Ue./Len)',(Ve./Len)',0.8,'k-')
hold off, axis equal, axis([0 lx 0 ly])
p = sort(p); caxis(p([8 end-7]))

title(sprintf('R = %0.1g t = %0.2g',R,k*dt))
colorbar
drawnow
end
end
